...
class pyRM114:
    def __init__(self, *args, directory='.', algorithm='osb unique microgroom'):
        ...
        __create_crm_files(categories, algorithm)
    ...
    def train_command(self, *args,
            ...
            algorithm='osb unique microgroom'args):
            train_method='TET') 
        ...
        if (not 0.0 <= train_partition <= 1.0):
            raise ValueError('Train partition must be between 0.0 and 1.0')

        #--algorithm
        __create_crm_files(file_names, algorithm)

        training_tweets = []
        test_tweets = []

        #if test mode, divide data set into training, testing, and CV sets
        ...
        for someones_tweets in training_tweets:
            ...
            someones_tweets_file = write_tweets_to_file(someones_tweets, self.directory, screen_name + '.txt')
    ...
    def __create_crm_files(file_names, classification_type):
